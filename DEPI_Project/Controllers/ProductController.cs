...
public async Task<IActionResult> AddProduct(Product model, IFormFile ProfilePicture)
{
    ...
    if (businessOwner == null)
    {
		...
		if (ProfilePicture != null && ProfilePicture.Length > 0)
		{
			...
			await _context.SaveChangesAsync();// Store file path in database
		}
	...
	public async Task<IActionResult> Details(int? id)
	{
		...
		var pro = await _context.Products.Include(p => p.Category)
